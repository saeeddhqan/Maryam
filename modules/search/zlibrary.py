...
meta = {
	...
	'options': (
		...
		('exact', False, False, 'Search for exact query(True or False, default=False)', '-e', 'store', bool),
...
def table_format(data):
	output_table = [] #Declaring output_table here in case no table found on page, return this one
	raw_data = BeautifulSoup(data, 'html.parser')

	for table in raw_data.find_all('table', {'class': 'table_book'}):
	
		if output_table == []: #If this statement is not used, table will reset each time and only 50 entries will be there
			output_table = [ {header.text:[]} for header in table.findAll('th', {'class': 'header'}) ]
		
		for row in table.find_all('tr'):
			for row_data, column in zip(row.find_all('td'), output_table): #Using zip to avoid cartesian product caused by 2 loops
				row_data = " ".join(row_data.text.split()) #Using split to clean the data and remove trailing spaces, /n, etc.
				key = [ i for i in column.keys() ][0] #Find the key of present column
				column[key].append(row_data) #Adding entire row data in one dump to a column		
	return output_table
...
def module_run(self):
	...
	for src in ['books', 'articles']:
		...
		columns = [j for i in base for j in i.keys() ] #Populating table headers
		
		for row_index in range(0, no_of_rows): #Using double loop to format data for table
			temp = []
