...
class Module(BaseModule):
    ...
    meta = {
        ...
        "options": (
            ("host", BaseModule._global_options["target"], True, "Host name"),
            ("logs", False, False, "Log files search(with 7 payload)"),
            ("backup", False, False, "Backup files search(with 148 payload)"),
            ("apache", False, False, "Apache status check(with 5 payload"),
            ("admin", False, False, "Admin panel check(with 482 payload)"),
            ("wordlist", os.path.join(BaseModule.data_path, 'FILENAME'), False, "Payload address file for url brute force attack(with \\n separator)"),
            ("brute", False, False, "brute force attack flag"),
        )
    }

    def module_run(self):
        url = self.urlib(self.options["host"]).sub_service("http")
        ### General Files ####
        ######################
        gens = {
        ...
        if self.options["brute"]:
            if not self.options["wordlist"]:
                self.error("\"wordlist\" option is require")
                return
            max_attempt = 4
            attempt = 0
