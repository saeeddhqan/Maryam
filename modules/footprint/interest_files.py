...
class Module(BaseModule):
	...
	meta = {
		...
		'options': (
			...
			('logs', False, False, 'Log files search(7 payload)', '--logs', 'store_true'),
			('backup', False, False, 'Backup files search(148 payload)', '--backup', 'store_true'),
	...
	def default(self, hostname, item, method, header, content, status_codes, not_status_codes):
		...
		try:
			...
			if all(conds):
				...
				self.resp[method].append(urjoin)
			else:
	...
	def module_run(self):
		...
		if self.options['logs']:
			...
			self.thread(self.default, url, log_payloads, thread_count, 'logs', status_codes=[200, 201, 204, 202, 206], header=('Content-Type', 'text/plain'))
		...
		if self.options['soap']:
			self.verbose('[SOAP] it will be helpful in attacking both SOAP and REST-based web services')
			self.resp['soap'] = []
		...
		if self.options['admin']:
			...
			self.alert('Admin Panel')
			self.thread(self.default, url, admin_payloads, thread_count, 'admin', status_codes=[200, 401, 403, 201, 202, 204, 206])
