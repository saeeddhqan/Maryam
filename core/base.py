...
class Base(framework.Framework):
    ...
    def _init_global_options(self):
        ...
        self.register_option(
            'agent', 'Mozilla/5.0 (X11; Linux x86_64; rv:75.0) Gecko/20100101 Firefox/75.0', True, 'user-agent string')
        self.register_option(
        ...
        self.register_option('history', True, False,
                             'logging all console inputs')
        self.register_option('update_check', True, False,
    ...
    def _check_version(self):
        if self._global_options.get('update_check'):
            ...
            if remote != local:
                self.alert(
                    'Your version of Maryam does not match the latest release')
    ...
    def _load_module(self, dirpath, filename):
        ...
        try:
            ...
            self.error(
                f"Module \'{mod_dispname}\' disabled. Dependency required: {self.to_unicode_str(e)[16:]}")
        except:
    ...
    def init_workspace(self, workspace):
        ...
        workspace = os.path.join(
            self._home, self.workspaces_dirname, workspace)
        if not os.path.exists(workspace):
            os.makedirs(workspace)

        # set workspace attributes
        self.workspace = framework.Framework.workspace = workspace
        self.prompt = self._prompt_template % (
            self._base_prompt[:-3], self.workspace.split('/')[-1])
    ...
    def show_banner(self):
        ...
        if self.loaded_category == {}:
            ...
            for count in sorted(counts, reverse=True):
                ...
                print(
                    f'{framework.Colors.B}{cnt.ljust(count_len + 2)} {mod_name} modules{framework.Colors.N}')
        print('')
    ...
    def help_workspaces(self):
        ...
        print(
            f'{os.linesep}Usage: workspaces [add|select|delete|list]{os.linesep}')
        ...
        if arg == 'list':
            ...
            if len(params) == 1:
                if not self.init_workspace(params[0]):
                    self.output(
                        f"Unable to initialize '{params[0]}' workspace.")
            else:
    ...
    def opt_proc(self, tool_name, args=None, output=None):
        ...
        for option in opts:
            try:
                ...
                self.error(
                    f"{tool_name}CodeError: options is too short. need more than {len(option)} option")
                return
            name = f"--{name}" if not name.startswith('-') else name
            try:
                parser.add_argument(
                    op, name, help=desc, dest=name, default=val, action=act, required=req)
    ...
    def run_tool(self, func, tool_name, args, output=None):
        try:
            proc = Process(target=getattr(self, func),
                           args=(tool_name, args, output))
            proc.start()
            proc.join()
            # python > 3.7
            if 'kill' in dir(proc):
