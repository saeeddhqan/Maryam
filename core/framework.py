...
class Framework(cmd.Cmd):
    ...
    def table(self, data, header, title='', linear=False, sep='-'):
        ...
        for i in range(0, cols):
            lens.append(len(max([self.to_unicode_str(
                x[i]) if x[i] != None else '' for x in tdata], key=len)))
    ...
    def exporter(self, datasets, filename, method):
        ...
        self.debug(
            f'Making a report with {method} format and save to the {filename}')
    ...
    def _get_history(self):
        ...
        commands = [f'{num}  {name}' for num,
                    name in enumerate(commands) if name]
        return commands
    ...
    def request(self, url, method='GET', **kwargs):
        ...
        if self._global_options['rand_agent']:
            ...
            headers['user-agent'] = headers.get('user_agent',
                                                False) or self._global_options['agent']
    ...
    def show_options(self, options=None):
        ...
        if options:
            ...
            print(
                f'{os.linesep}{self.spacer}No options available for this module.{os.linesep}')
        ...
        if arg[:1] == '$':
            ...
            if len(params) == 2:
                if params[1] in ["limit", "proxy", "target", "timeout", "agent", "verbosity", "history"]:
                    self.error(
                        f"You cannot delete default variable '{params[1]}'.")
                else:
                    if self.delete_var(params[1][1:]):
                        self.output(f"Var '{params[1]}' deleted.")
                    else:
                        self.error(
                            f"No such var was found for deletion '{params[1]}'.")
            else:
            ...
            if not Framework._record:
                if len(arg.split()) > 1:
                    ...
                    if not self._is_readable(filename, 'w'):
                        self.output(
                            f'Cannot record commands to \'{filename}\'.')
                    else:
                        Framework._record = filename
                        self.output(
                            f'Recording commands to \'{Framework._record}\'.')
                else:
                    self.help_record()
            else:
            ...
            if not Framework._spool:
                if len(arg.split()) > 1:
                    ...
                    if not self._is_readable(filename, 'w'):
                        ...
                        Framework._spool = codecs.open(
                            filename, 'ab', encoding='utf-8')
                        self.output(
                            f'Spooling output to \'{Framework._spool.name}\'.')
                else:
                    self.help_spool()
            else:
    ...
    def help_history(self):
        ...
        print(
            f'{os.linesep}Usage: history [list|from <num>|off|on|status|all|clear]')
        ...
        print(
            f'Note: If \'from <num>\' is not set, only the last 50 commands will be shown.{os.linesep}')
    ...
    def help_var(self):
        ...
        print(
            f'{os.linesep}Usage: var <$name> <value> || var [delete] <name> || var [list]{os.linesep}')

    def help_record(self):
        print(getattr(self, 'do_record').__doc__)
        print(
            f'{os.linesep}Usage: record [start <filename>|stop|status]{os.linesep}')

    def help_spool(self):
        print(getattr(self, 'do_spool').__doc__)
        print(
            f'{os.linesep}Usage: spool [start <filename>|stop|status]{os.linesep}')

    def help_report(self):
        print(getattr(self, 'do_report').__doc__)
        print(
            f'{os.linesep}Usage    : report [<format> <filename> [<module_name> or <module_name> <query(hostname,domain name, keywords,etc)>]]')
