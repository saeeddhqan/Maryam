...
class main:
	def __init__(self, q, limit=50, session_id=''):
		...
		self.headers = {'User-Agent': self.framework.rand_uagent().get,
	...
	def get_similar_users(self):
		...
		r = requests.get(url=f"{url}{self.q}", headers=self.headers)
	...
	def get_user_posts(self):
		...
		if self.posts_next_page == None:
			payload = f'\{"id": \"{self._userdata["id"]}\", "first": 50, "after": \"{self.posts_next_page}\"\}'
